...
- `RoutineTemplateExercise` (id, templateId, exerciseId, orden, series, repeticiones, peso nullable, notas)
- `RoutineInstance` (id, gymId, **profesorId nullable**, alumnoId, nombre, origenTemplateId nullable, **vinculada**, vigenteDesde, vigenteHasta nullable, activa) — `origenTemplateId` nullable porque el profesor puede armar una instancia desde cero sin partir de plantilla. `profesorId` pasa a nullable (`onDelete: SetNull`) por la cascada de hard-delete de PROFESOR: la instancia puede sobrevivir a la eliminación de quien la creó, si el alumno tiene otro profesor vigente en cartera. `vinculada` (boolean): si el profesor optó por mantener la instancia sincronizada con su plantilla de origen al asignarla, una edición posterior de esa plantilla se propaga a la instancia — hasta que el profesor edite los ejercicios de esa instancia puntualmente y el conjunto diverja del de la plantilla, momento en que se desvincula automáticamente (`vinculada: false`) para no perder la personalización.
- `RoutineInstanceExercise` (id, instanceId, exerciseId, orden, series, repeticiones, peso nullable, notas)
