...
- Al asignar, el profesor manda los días ya armados (desde plantilla completa, días sueltos de plantillas o desde cero); el servidor los persiste y decide el vínculo de cada uno con la regla de arriba.
...
- **Endpoints:** `POST/GET /routine-templates`, `GET/PATCH/DELETE /routine-templates/:id`, `PUT /routine-templates/:id/dias` (PROFESOR, sin cartera); `POST /routine-instances`, `GET/PATCH /routine-instances/:id`, `PUT /routine-instances/:id/dias` (responde `{ diasDesvinculados }`), `GET /users/:alumnoId/rutina-vigente` (PROFESOR, con cartera); `GET /users/me/rutina-vigente` (ALUMNO) — resuelve los ejercicios contra `ExerciseRepositoryPort.findByIds`, nunca expone solo el `exerciseId`. `rutina-vigente` devuelve `dias[]` (la vista PROFESOR incluye `vinculado` por día).
