'use client';

import { useFormStatus } from 'react-dom';
import { useActionState } from 'react';
import { createProfesorAction, CreateProfesorActionState } from './actions';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Field, FieldLabel } from '@/components/ui/field';
import { Alert, AlertDescription } from '@/components/ui/alert';
import { Spinner } from '@/components/ui/spinner';

const ESTADO_INICIAL: CreateProfesorActionState = { error: null, success: false };

function BotonCrear() {
  const { pending } = useFormStatus();
  return (
    <Button type="submit" variant="brand" disabled={pending} className="w-full sm:w-auto">
      {pending && <Spinner data-icon="inline-start" />}
      {pending ? 'Creando...' : 'Crear profesor'}
    </Button>
  );
}

export function CreateProfesorForm() {
  const [estado, formAction] = useActionState(createProfesorAction, ESTADO_INICIAL);

  return (
    <section className="rounded-2xl bg-background p-4 shadow-xs sm:p-6">
      <h2 className="mb-4 text-lg font-semibold text-foreground">Nuevo profesor</h2>
      <form action={formAction} className="flex flex-col gap-3">
        <Field>
          <FieldLabel htmlFor="admin-profesor-username" className="sr-only">
            Usuario
          </FieldLabel>
          <Input
            id="admin-profesor-username"
            name="username"
            placeholder="Usuario"
            required
            minLength={3}
            autoCapitalize="none"
            autoCorrect="off"
          />
        </Field>
        <Field>
          <FieldLabel htmlFor="admin-profesor-nombre" className="sr-only">
            Nombre
          </FieldLabel>
          <Input
            id="admin-profesor-nombre"
            name="nombre"
            placeholder="Nombre"
            required
            minLength={2}
          />
        </Field>
        <Field>
          <FieldLabel htmlFor="admin-profesor-password" className="sr-only">
            Contraseña
          </FieldLabel>
          <Input
            id="admin-profesor-password"
            name="password"
            type="password"
            placeholder="Contraseña"
            required
            minLength={6}
          />
        </Field>
        <BotonCrear />
        {estado.error && (
          <Alert variant="destructive">
            <AlertDescription>{estado.error}</AlertDescription>
          </Alert>
        )}
        {estado.success && <p className="text-sm text-success">Profesor creado.</p>}
      </form>
    </section>
  );
}
